...
public:
    ...
    Food(string n = "Еда") {
        this->name = n;
        // Комментарий: Можно добавить вывод конструктора для отладки, если нужно
    ...
    virtual void printInfo() const {
        cout << "Это объект Food (Еда): " << name << endl;
    }
};
...
public:
    ...
    Fruit(string n = "Фрукт") : Food(n) { // <-- Food(n) остается здесь!
    ...
    void printInfo() const override {
        cout << "Это объект Fruit (Фрукт): " << name << endl;
    }
...
public:
    ...
    Vegetable(string n = "Овощ") : Food(n) { // <-- Food(n) остается здесь!
    ...
    void printInfo() const override {
        cout << "Это объект Vegetable (Овощ): " << name << endl;
    }
...
void tryUnsafeCastToFruit(Food* ptr) {
    cout << endl << "Попытка НЕБЕЗОПАСНОГО приведения к Fruit* " << endl; // Используем endl
    Fruit* unsafe_fruit_ptr = static_cast<Fruit*>(ptr);
    cout << "Небезопасное приведение выполнено (static_cast)." << endl; // Используем endl
    if (ptr->classname() != "Fruit") {
        cout << "Попытка вызова метода Fruit::peel() через неверно приведенный указатель может вызвать краш!" << endl; // Используем endl
    }
    else {
        cout << "  (Тип оказался верным, вызов метода отработает, но это все равно плохая практика без проверки)" << endl; // Используем endl
...
int main() {
    ...
    foods.push_back(make_unique<Fruit>("Банан"));

    cout << "Обработка продуктов " << endl; // Используем endl
    for (const auto& food_uptr : foods) {
        Food* ptr = food_uptr.get(); // Получаем сырой указатель для работы

        cout << endl << "Информация об объекте:" << endl; // Используем endl
        ptr->printInfo(); // Полиморфный вызов, printInfo() использует endl

        // 1. Использование classname()
        cout << "  classname(): " << ptr->classname() << endl; // Используем endl

        // 2. Использование isA()
        // Используем std::boolalpha для вывода true/false вместо 1/0
        cout << "  isA(\"Food\"):      " << boolalpha << ptr->isA("Food") << endl; // Используем endl
        cout << "  isA(\"Fruit\"):     " << boolalpha << ptr->isA("Fruit") << endl; // Используем endl
        cout << "  isA(\"Vegetable\"): " << boolalpha << ptr->isA("Vegetable") << endl; // Используем endl
        ...
        cout << "Попытка ручного безопасного приведения (с isA) " << endl; // Используем endl
        if (ptr->isA("Fruit")) {
            Fruit* fruit_ptr_manual = static_cast<Fruit*>(ptr);
            cout << "  Ручное приведение к Fruit* успешно (использовали isA)." << endl; // Используем endl
            fruit_ptr_manual->peel(); // peel() использует endl
        }
        else {
            cout << "  Ручное приведение к Fruit* не удалось (объект не Fruit)." << endl; // Используем endl
        }

        // 5. Безопасное приведение типов (dynamic_cast)
        cout << "Попытка dynamic_cast " << endl; // Используем endl
        // Попытка приведения к Fruit*
        Fruit* fruit_ptr_dynamic = dynamic_cast<Fruit*>(ptr);
        if (fruit_ptr_dynamic != nullptr) {
            cout << "  dynamic_cast к Fruit* успешен." << endl; // Используем endl
            fruit_ptr_dynamic->peel(); // peel() использует endl
        }
        else {
            cout << "  dynamic_cast к Fruit* не удался (вернул nullptr)." << endl; // Используем endl
        }
        ...
        if (veg_ptr_dynamic) { // Краткая проверка на nullptr
            cout << "  dynamic_cast к Vegetable* успешен." << endl; // Используем endl
            veg_ptr_dynamic->wash(); // wash() использует endl
        }
        else {
            cout << "  dynamic_cast к Vegetable* не удался." << endl; // Используем endl
        }
    }

    // Очистка памяти не нужна вручную, т.к. использовали unique_ptr
    cout << endl << "Объекты будут автоматически удалены unique_ptr при выходе из main" << endl; // Используем endl
