...
public:
    ...
    Ingredient(const string& n) {
        ...
        cout << "Ингредиент '" << name << "' получен (Конструктор)" << endl; 
    }

    // Деструктор
    ~Ingredient() {
        cout << "Ингредиент '" << name << "' выброшен (Деструктор)" << endl; 
    }

    // Метод use
    void use() const {
        cout << "Используем ингредиент'" << name << "'" << endl; 
    }
...
void process_unique(unique_ptr<Ingredient> unique_ing) {
    cout << "Внутри process_unique" << endl; 
    if (unique_ing) {
        cout << "Получен unique_ptr (владение передано)" << endl; 
        unique_ing->use(); // use() использует endl
    }
    else {
        cout << "Получен пустой unique_ptr" << endl; 
    }
    cout << "Выход из process_unique" << endl; 
} // unique_ing выходит из области видимости, Ингредиент удаляется
...
void share_ingredient(shared_ptr<Ingredient> shared_ing) {
    cout << "Внутри share_ingredient" << endl; 
    if (shared_ing) {
        cout << "Получен shared_ptr. Счетчик ссылок: " << shared_ing.use_count() << endl; 
        shared_ing->use(); // use() использует endl
    }
    else {
        cout << "Получен пустой shared_ptr" << endl; 
    }
    cout << "Выход из share_ingredient" << endl; 
...
void observe_shared(const shared_ptr<Ingredient>& shared_ing_ref) {
    cout << "Внутри observe_shared" << endl; 
    if (shared_ing_ref) {
        cout << "Получена ссылка на shared_ptr. Счетчик ссылок: " << shared_ing_ref.use_count() << endl; 
        shared_ing_ref->use(); // use() использует endl
    }
    else {
        cout << "Получена пустая ссылка на shared_ptr" << endl; 
    }
    cout << "Выход из observe_shared" << endl; 
...
int main() {
    ...
    cout << "Демонстрация unique_ptr" << endl << endl; 

    cout << "Создание unique_ptr" << endl; 
    ...
    cout << endl << "Перемещение unique_ptr" << endl; 
    unique_ptr<Ingredient> uptr_moved_to;

    cout << "До перемещения: uptr_carrot " << (uptr_carrot ? "владеет ресурсом" : "пустой") << ", uptr_moved_to " << (uptr_moved_to ? "владеет ресурсом" : "пустой") << endl; 

    uptr_moved_to = move(uptr_carrot);

    cout << "После перемещения: uptr_carrot " << (uptr_carrot ? "владеет ресурсом" : "пустой") << ", uptr_moved_to " << (uptr_moved_to ? "владеет ресурсом" : "пустой") << endl; 

    if (uptr_moved_to) uptr_moved_to->use(); // use() использует endl

    cout << endl << "Передача unique_ptr в функцию (process_unique)" << endl; 
    process_unique(move(uptr_onion)); // Внутри process_unique используются endl

    cout << "После process_unique: uptr_onion " << (uptr_onion ? "владеет ресурсом" : "пустой") << endl; 

    cout << endl << "Получение unique_ptr из функции" << endl; 
    auto uptr_potato = buy_unique_ingredient("Картофель"); // Внутри buy_unique_ingredient используются endl
    cout << "В main, после buy_unique_ingredient:" << endl; 
    if (uptr_potato) uptr_potato->use(); // use() использует endl

    cout << endl << "unique_ptr выходят из области видимости" << endl; 
    // Деструкторы выведут сообщения с endl

    cout << endl << endl << "Демонстрация shared_ptr" << endl << endl; 

    shared_ptr<Ingredient> sptr_salt;

    cout << "Создание shared_ptr" << endl; 
    ...
    cout << endl << "Копирование shared_ptr" << endl; 
    ...
    cout << endl << "Передача shared_ptr в функции" << endl; 
    cout << "Вызов share_ingredient (передача по значению - копия увеличит счетчик):" << endl; 
    ...
    cout << endl << "Сброс shared_ptr (reset)" << endl; 
    ...
    cout << "После sptr_salt_copy.reset():" << endl; 
    cout << "sptr_salt - счетчик ссылок: " << sptr_salt.use_count() << endl;      // = 1
    cout << "sptr_salt_copy " << (sptr_salt_copy ? "владеет ресурсом" : "пустой") << endl; 


    cout << endl << "shared_ptr выходят из области видимости" << endl; 
